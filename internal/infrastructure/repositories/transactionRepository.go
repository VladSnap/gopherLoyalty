...
func (r *TransactionImplRepository) FindByID(ctx context.Context, id string) (*domain.Transaction, error) {
...
func (r *TransactionImplRepository) CalcBalance(ctx context.Context, userID string) (domain.CurrencyUnit, error) {
	var balance int
	query := `SELECT SUM(CASE WHEN type = 'ACCRUAL' THEN amount ELSE -amount END) AS balance
        FROM transactions t
        JOIN orders o ON t.order_id = o.id
        WHERE o.user_id = $1`
	err := r.db.GetContext(ctx, &balance, query, userID)
	if err != nil {
		return domain.CurrencyUnit(0), errors.Wrap(ErrDatabase, "failed to find loyalty account transaction by ID")
	}

	return domain.CurrencyUnit(balance), nil
}
